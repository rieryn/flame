...
class RedWarrior extends SpriteAnimationComponent {
  ...
  void boundsCollision(){
    ...
    if(position.y < topBound){stop();position = Vector2(position.x,topBound);bounce(Vector2(0,1),0.1,wallBounces);}
    if(position.y > bottomBound){stop();position = Vector2(position.x,bottomBound);bounce(Vector2(0,-1),0.1,wallBounces);}
  }
  void checkDead(){
    if(hp<0){
      invuln = true;
